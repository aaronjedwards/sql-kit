...
/// example, ``SQLIdentifier`` uses the serializer's dialect when quoting identifiers (naturally enough). Many
/// ``SQLExpression``s - especially those representing entire SQL queries, such as ``SQLSelect`` or
/// ``SQLCreateTable`` - function solely as containers of other expressions which are serialized in an appropriate
/// sequence.
///
